...
namespace strutil
{
    static inline std::vector<std::string> get_files(std::string& dir_in) {
        DIR* dir;
        struct dirent* ent;
        std::vector<std::string> files;
        if ((dir = opendir(dir_in.c_str())) != NULL) {
            while ((ent = readdir(dir)) != NULL) {
                std::string name = ent->d_name;
                if (name.compare(".")!=0 && name.compare("..")!=0) {  
                    std::string join_path = dir_in + "\\" + name;
                    files.push_back(join_path);
                }
            }
            closedir(dir);
        }
    ...
    static bool check_extention(std::string& fname, std::string& extension) {
