...
void icpExample()
{
	...
	for (int i = 0; i < dynamicPointCloud.size(); i++)
	{
		alignmentError += pow(dynamicPointCloud[i]->pos[0] - staticPointCloud[i]->pos[0], (double)2.0);
		alignmentError += pow(dynamicPointCloud[i]->pos[1] - staticPointCloud[i]->pos[1], (double)2.0f);
		alignmentError += pow(dynamicPointCloud[i]->pos[2] - staticPointCloud[i]->pos[2], (double)2.0f);
	}
...
void main()
{
	...
	std::string pts1_path = "N:\\tasks\\reg\\tile_250m\\tile_2ref.txt";
	std::string pts2_path = "N:\\tasks\\reg\\tile_250m\\tile_2src.txt";
	std::vector<Point*> staticPointCloud;
	std::vector<Point*> dynamicPointCloud;
	...
	icp_para.max_distance = 20;
	icp_para.trans_type = RIGID;
	icp_para.reject_type = Trimmed_Distance;
	ICPResult result;
	icp(dynamicPointCloud, staticPointCloud,icp_para,result);
